...
lambdaseq = [0:0.1:1];
lambdavarseq = [0:0.1:1];

corrmat = zeros(length(lambdavarseq),length(lambdaseq));
uncertaintymat = zeros(length(lambdavarseq),length(lambdaseq));
errormat = zeros(length(lambdavarseq),length(lambdaseq));

for lam = lambdaseq
    for lamvar =  lambdavarseq
%Dummy data
samples =[];
Ntrials = 1000;
Nelectrodes = 2;
...
for l = 1:Nelectrodes
    pref = rand(1)*2*pi;
    for k = 1:trialLength
        samples(k,l,:) = abs(stimval-pref)*signalstrength + noisestrength*randn(Ntrials,1);
    end
end
...
uncertaintymat(find(lambdavarseq == lamvar), find(lambdaseq == lam)) = avgunc;
errormat(find(lambdavarseq == lamvar), find(lambdaseq == lam)) = avgerror;
corrmat(find(lambdavarseq == lamvar), find(lambdaseq == lam)) = corr(errors,uncertainty);
...
legendlist = strings(1,length(lambdavarseq));
for i = [1:length(lambdavarseq)]
    legendlist(i) = sprintf('lambda_var = %f', lambdavarseq(i));
end

figure
plot(lambdaseq, errormat)
legend(legendlist)
title ('avg decoder error by lambda');

figure
plot(lambdaseq, uncertaintymat)
legend(legendlist)
title ('avg uncertainty by lambda');

figure
plot(lambdaseq, corrmat)
legend(legendlist)
title ('trial-by-trial correlation between uncertainty and decoder error by lambda');
