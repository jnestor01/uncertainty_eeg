...
cd (homefolder);
parentfolder = nextfolder('sim_11_1_');
parent = mkdir(parentfolder);
home = cd(parentfolder);
%% Start simulation iteration loop
for w = 1:p.niters
    sim.seed = randi(2^10,1);
    rng(sim.seed, 'twister');
foldername = sprintf('iter%d', w);
...
Nelectrodes = sim.p.Nelectrodes;
timewindow = sim.p.timewindow;

allsamples = zeros(size(timewindow,2)+1,Nelectrodes,Ntrials);
t0 = sim.p.tzero;

tuning_corr_noise = sim.p.tuning_corr_noise;
ind_noise = sim.p.ind_noise;
global_noise = sim.p.global_noise;
decay = sim.p.decay;

signalstrength = sim.p.signalstrength;
...
sim.postmat(:,:,m) = posteriors; 

end

sim.avgerror = mean(sim.errmat);
sim.avgeunc = mean(sim.uncmat);

pooledpost = mean(sim.postmat, 3);
...
% clear plo
% for m=1:t0
% pl = plot(binvals/2, sim.postmat(plottrial,:,m), 'color', col(1));
% plo(m) = pl(1); hold on
% end
% for m=t0+1:timewindow(end)
% pl = plot(binvals/2, sim.postmat(plottrial,:,m), 'color', col(2)); 
% plo(m) = pl(1); hold on
% end
...
% 
% col = ['red', 'blue'];
% pre = scatter(sim.errmat(1:t0),sim.uncmat(1:t0), 'Color', col(1)); hold on
% post = scatter(sim.errmat(t0+1:end), sim.uncmat(t0+1:end), 'Color', col(2));
% ylabel('decoded uncertainty');
...
function name = nextfolder(basename)
suffix = 0;
brk = 0;
while ~brk
    if isfolder(sprintf('%s%s', basename, num2str(suffix)))
        suffix = suffix+1;
    else
        brk = 1;
    end
end
name = sprintf('%s%s', basename, num2str(suffix));
end