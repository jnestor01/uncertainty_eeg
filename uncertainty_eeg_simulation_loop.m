...
corrmat = zeros(length(sigseq),length(noiseseq));
uncertaintymat = zeros(length(sigseq),length(noiseseq));
errormat = zeros(length(sigseq),length(noiseseq));
...
uncertaintymat(find(sigseq == signalstrength), find(noiseseq == noisestrength)) = avgunc;
errormat(find(sigseq == signalstrength), find(noiseseq == noisestrength)) = avgerror;
corrmat(find(sigseq == signalstrength), find(noiseseq == noisestrength)) = corr(errors,uncertainty);
...
legendlist = strings(1,length(noiseseq));
for i = [1:length(noiseseq)]
    legendlist(i) = sprintf('noise factor = %f', noiseseq(i));
end

figure
plot(sigseq, errormat)
legend(legendlist)
title ('avg decoder error by signal strength');

figure
plot(sigseq, uncertaintymat)
legend(legendlist)
title ('avg uncertainty by signal strength');

figure
plot(sigseq, corrmat)
legend(legendlist)
title ('trial-by-trial correlation between uncertainty and decoder error by signal strength');
